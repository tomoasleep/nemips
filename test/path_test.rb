...
VhdlTestScript.scenario "../src/path.vhd" do |dut|
  ...
  step fsm.state => "state_mem_read", mem.address_in => 0x8, mem.write_enable => 0
  ...
  step fsm.state => "state_mem_read", mem.address_in => 0x10000, mem.write_enable => 0
  step fsm.state => "state_mem_wbx", reg.a3 => 3, mem.read_data => 0x2222,
    reg.wd3 => 0x2222

  # memory write x
  step fsm.state => "state_fetch",
    mem.read_data => instruction_r("i_op_r_group", 4, 5, 6, 0, "r_fun_swx")
  ...
  step fsm.state => "state_mem_writex", mem.write_enable => 1, mem.address_in => 0x999,
    mem.write_data => 0xf, alu.result => 0x999

  # alu
  step fsm.state => "state_fetch", mem.read_data => instruction_r("i_op_r_group", 1, 2, 3, 0, "r_fun_add"),
    mem.write_enable => 0, reg.we3 => 0
  ...
  # jmp
  step fsm.state => "state_fetch", mem.read_data => instruction_j("j_op_j", 0x100),
  ...
  # jal
  step fsm.state => "state_fetch", mem.read_data => instruction_j("j_op_jal", 0x200),
    pc.pc => 0x10000010, alu.a => 0x40000040, alu.b => 0x4,
    alu.result => 0x40000044, alu.alu_ctl => "alu_ctl_add",
    pc.write_data => 0x10000011, pc.pc_write => 1
  ...
  # jmpr
  step fsm.state => "state_fetch",
    mem.read_data => instruction_r("i_op_r_group", 5, 9, 3, 0, "r_fun_jr"),
  ...
  # jalr
  step fsm.state => "state_fetch",
    mem.read_data => instruction_r("i_op_r_group", 1, 2, 3, 0, "r_fun_jalr"),
